...
ADMIN_ID = 1375000407
# НАСТРОЙКА: Часовой пояс твоих пользователей (например, 5 для UTC+5, 3 для UTC+3)
USER_TZ = 5 
...
def parse_custom_datetime(user_input: str) -> datetime:
    """Умный парсер даты и времени с учетом часового пояса пользователя"""
    # Берем текущее время в UTC и переводим в часовой пояс пользователя для логики "сегодня/завтра"
    ...
    # 2. Проверяем относительные слова
    ...
    # Собираем datetime в часовом поясе пользователя
    ...
    # Если время в сутках уже прошло — переносим на завтра
    if result_datetime.timestamp() <= now_user.timestamp() and "понедельник" not in user_input: 
...
async def save_reminder(message: Message, state: FSMContext):
    ...
    if not reminder_datetime:
        await message.answer("❌ Не удалось понять время. Кажется, вы забыли указать ЧЧ:ММ.")
        return

    # Сохраняем в строку без привязки к TZ для отображения пользователю
    ...
    repeat_keyboard = ReplyKeyboardMarkup(
        keyboard=[[KeyboardButton(text="❌ Один раз")], [KeyboardButton(text="🔁 Каждый день")]],
        resize_keyboard=True
    )
    await message.answer("🔁 Какое напоминание создать?", reply_markup=repeat_keyboard)
...
async def process_repeat_choice(message: Message, state: FSMContext):
    ...
    # Логика расчета секунд ДО напоминания (переводим всё строго в UTC timestamp)
    tz_user = timezone(timedelta(hours=USER_TZ))
    ...
    print(f"=== Создание напоминания ===")
    print(f"text={text} | seconds={seconds_to_wait}")

    asyncio.create_task(
        send_reminder_by_delay(message.chat.id, message.from_user.id, text, remind_time_str, seconds_to_wait, is_daily)
    )

    await message.answer(
        f"✅ Напоминание успешно создано!\n\n📝 {text}\n📅 Будет отправлено: {remind_time_str}\n🔁 Тип: {message.text}",
        reply_markup=main_keyboard
    )
    await state.clear()

async def send_reminder_by_delay(chat_id, user_id, text, remind_time_str, seconds, is_daily=0):
    print(f"=== send_reminder_by_delay стартовала: ждать {seconds} сек ===")
    try:
        if seconds > 0:
        ...
        if is_daily:
            ...
            # Ровно через сутки (86400 секунд) запускаем таск заново
            asyncio.create_task(
                send_reminder_by_delay(chat_id, user_id, text, new_time_str, 86400, is_daily=1)
            )
        else:
    ...
    try:
        ...
        for reminder in reminders:
            ...
            try:
                ...
                # Передаем max(0, seconds), чтобы пропущенные за время офлайна напоминания отправились сразу
                asyncio.create_task(
